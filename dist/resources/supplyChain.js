...
export function supplyChainResources(server) {
    ...
    server.resource("supplier-performance", new ResourceTemplate("supply-chain://performance/{supplierId}", { list: undefined }), async (uri, { supplierId }) => {
        ...
        const metrics = {
            ...
            onTimeDelivery: supplierOrders.filter(order => order.status === "delivered" &&
                order.actualDelivery &&
                new Date(order.actualDelivery) <= new Date(order.expectedDelivery)).length,
    ...
    // Get pending orders
    server.resource("pending-orders", "supply-chain://orders/pending", async (uri) => {
