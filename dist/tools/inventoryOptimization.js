import { z } from "zod";
export function inventoryOptimizationTools(server) {
    // Calculate Economic Order Quantity (EOQ)
    server.tool("calculate-eoq", {
...
Time Between Orders: ${optimalTimeBetweenOrders.toFixed(2)} days`
    ...
    server.tool("calculate-safety-stock", {
...
Reorder Point: ${Math.round(reorderPoint).toLocaleString()} units`
    ...
    server.tool("optimize-inventory", {
        ...
        const salesStdDev = Math.sqrt(monthlySales.reduce((sq, n) => sq + Math.pow(n - averageSales, 2), 0) / monthlySales.length);
...
//# sourceMappingURL=inventoryOptimization.js.map