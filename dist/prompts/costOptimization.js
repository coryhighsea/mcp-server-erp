import { z } from "zod";
export function costOptimizationPrompts(server) {
    // Cost structure analysis prompt
    server.prompt("analyze-cost-structure", "Analyze product cost structure and identify optimization opportunities", {
        productId: z.string(),
        costData: z.string()
    }, (args) => ({
        messages: [{
...
5. Implementation strategy for cost optimization`
    ...
    server.prompt("analyze-supplier-costs", "Analyze supplier costs and performance metrics", {
        supplierId: z.string(),
        costData: z.string()
    }, (args) => ({
        messages: [{
...
5. Negotiation strategy for cost reduction`
    ...
    server.prompt("analyze-process-costs", "Analyze manufacturing and operational process costs", {
        processId: z.string(),
...
5. Implementation roadmap for process improvements`
    ...
    server.prompt("analyze-inventory-costs", "Analyze inventory holding costs and optimization opportunities", {
        productId: z.string(),
