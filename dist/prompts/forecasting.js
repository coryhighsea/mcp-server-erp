import { z } from "zod";
export function forecastingPrompts(server) {
    // Demand forecasting prompt
    server.prompt("forecast-demand", "Generate demand forecast based on historical data", {
        productId: z.string(),
...
5. Recommendations for inventory planning based on the forecast`
    ...
    server.prompt("analyze-seasonality", "Analyze seasonal patterns in sales data", {
        productId: z.string(),
        salesData: z.string()
    }, (args) => ({
        messages: [{
...
5. Suggestions for promotional timing based on seasonal patterns`
    ...
    server.prompt("analyze-trends", "Analyze sales trends and growth patterns", {
        productId: z.string(),
        historicalData: z.string()
    }, (args) => ({
        messages: [{
...
5. Strategic recommendations based on trend analysis`
    ...
    server.prompt("analyze-market-impact", "Analyze impact of market events on sales", {
        productId: z.string(),
        salesData: z.string()
    }, (args) => ({
        messages: [{
